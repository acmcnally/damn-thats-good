...
- **API** guard: when `E2E_AUTH_BYPASS=1` and the `e2e_bypass` cookie is present, skip WorkOS verification entirely and provision/return a fixed deterministic test user (`workos_user_id: 'e2e-test-user'`, `email: 'e2e@example.test'`).
- **No header-based sibling for direct `request.*` calls, deliberately.** The original DAMN-29 scaffold comment anticipated needing one, but DAMN-1 has no actual use for it: the one authenticated assertion in the test plan (confirming the fixed e2e user is signed in) is asserted through `page` — reading the rendered "signed in as ‹email›" text, which the cookie already covers — not through a direct API call. The existing `/api/health` `request.*` call is unauthenticated by design and needs no credential either. Feature assertions belong on `page`, not `request`, once there's a UI to assert through (that's the point of the workflow tier — ADR-0012); `request.*` stays reserved for infra-only checks with no UI surface and future test-data seeding, neither of which needs this header yet. Build it later, scoped to whatever real need creates it — most likely seeding once DAMN-2 has data worth seeding — rather than guess its shape now.
...
- **This is a standing state on staging, not a one-shot toggle — worth being explicit about.** Once set in staging's `deploy/.env`, `E2E_AUTH_BYPASS=1` stays on indefinitely (it isn't toggled per CI run). Unlike prod, the cookie *is* sufficient on staging by itself — any browser that can reach staging can self-authenticate as the fixed test user, no invite needed. Accepted: staging has no public ingress (tailnet-only, ADR-0010), and tailnet membership is restricted to the owner's own devices — this isn't a new hole, it's an existing trust boundary carrying one more thing. Stated here explicitly rather than left implicit, since it stops being a footnote if that tailnet-membership assumption ever changes.
...
- `/login` route: WorkOS's dashboard requires a registered **Initiate Login URI** that calls `signIn()` (used for WorkOS-*initiated* flows — admin impersonation, shared links — not our primary user-initiated flow, but AuthKit expects it to exist). Confirmed live: unlike redirect URIs, this is a **single-value** field, set to staging's host (`https://<staging-tailnet-hostname>/login`) — local dev has no real use case for it (nothing else can reach `localhost:5173` to trigger one of these flows against it). No router library is in the app today (`apps/web/src/App.tsx` renders unconditionally) and adding one is out of proportion for two auth-only paths — `/login` is distinguished from `/` with a plain `window.location.pathname` check, not a routing library. (DAMN-2 onward will likely want real client-side routing once there's more than one screen; not this issue's problem to solve early.) The `/callback` redirect URI needs no route component at all — `AuthKitProvider`'s own `onRedirectCallback` handling intercepts it directly, before app code sees it.
...
4. **Staging environment: register both** `http://localhost:5173/callback` (local dev) **and** `https://<staging-tailnet-hostname>/callback` (deployed staging) **as redirect URIs** (Applications → Damn That's Good → Redirects; a list) — both are needed on the *same* environment, since local dev and deployed staging share Staging's WorkOS keys (ADR-0010), and each running instance requests the one matching its own origin at runtime (nothing to choose between on our side — see Config / env changes below). Also set the **Initiate Login URI** — single-value, confirmed live — to `https://<staging-tailnet-hostname>/login`; local dev has no use case for this one.
   - **Also register both bare origins as CORS origins** (Applications → Damn That's Good → Sessions → CORS — a separate allowlist from Redirects above): `http://localhost:5173` and `https://<staging-tailnet-hostname>` (no `/callback` path, no trailing slash). Needed because `@workos-inc/authkit-react` is a client-only integration — the browser calls `api.workos.com/user_management/authenticate` directly to exchange the code, so an unlisted origin gets no `Access-Control-Allow-Origin` header, the exchange fails, and the symptom is a sign-in redirect loop rather than an explicit error.
   - **Also register both bare origins as the sign-out/logout redirect URI** — a fourth, separate allowlist from Redirects/CORS/Providers: `http://localhost:5173` and `https://<staging-tailnet-hostname>`. `signOut()` defaults to `returnTo: '/'`, which WorkOS checks against this list before completing sign-out; with nothing registered, sign-out lands on WorkOS's generic `error.workos.com` page instead of back in the app.
...
- **Workflow (Playwright):** replaces the DAMN-26 skeleton assertions — `loginAsTestUser` bypass gets past the auth wall, then the fixed e2e user's identity is confirmed **page-driven**, by reading the rendered "signed in as e2e@example.test" text on the landing page (not a direct `request.get('/api/me')` — see the E2E-bypass section above on why), plus sign-out (if reachable without real WorkOS — otherwise assert the sign-out button exists and calls the SDK method, without asserting the full round trip through a real WorkOS session). The `/api/health` check stays a direct `request.*` call, unauthenticated by design.
...
- E2E bypass mechanics: cookie-only (page-driven), pinned down above — the DAMN-29 scaffold comment anticipated a header-based sibling too, but DAMN-1 has no actual use for one; deferred until a real need (likely test-data seeding) creates it.

## Adversarial design review (2026-09-04) — resolved

A fresh-context review against the frozen scope, ADRs, and adjacent issues found two implementation-blocking gaps, one pre-flight check, and seven smaller findings. All resolved above, discussed individually with the owner rather than batch-applied: local `docker-compose.yml` env wiring, the `WorkosUserLookup` seam, the Magic-Auth/invite-only pre-flight check, stale `users.email` (accepted as a known limitation), frontend 401 handling, the `/login` routing mechanism (no router), the staging bypass's standing-backdoor tradeoff (accepted, tailnet-only), an explicit test for the bypass safety invariant, and dropping the speculative `X-E2E-Test-User` header. One idea that came out of the discussion but belongs to a different issue — a post-deploy prod smoke-test suite — is captured on DAMN-30, not built here.
