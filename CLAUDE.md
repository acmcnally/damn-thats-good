...
- **CI (ADR-0010, `.github/workflows/ci.yml`):** the `verify` job runs `pnpm verify` on every PR and is the required check for `main`; a merge to `main` also builds + publishes the `api` / `web` images to GHCR, then the `deploy-staging` job (DAMN-28) deploys them to the staging LXC over Tailscale SSH. `deploy/` holds the staging/prod compose + deploy script + runbook.
