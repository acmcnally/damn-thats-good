...
- **No Linear issue IDs in code or UI copy.** Comments must be self-contained — git blame → PR → design doc is the trail back to the *why*. The only exception: a genuine stub/seam for future work may carry the issue ID as a trailing pointer (e.g. `// injection point for cross-device sync (DAMN-14)`), and only if the comment already reads on its own. Never self-reference the issue being implemented; never cite a closed issue to explain existing behaviour; keep issue numbers out of rendered text (release names like "V2" are fine). In-repo pointers (`docs/features/<slug>/`, `docs/adr/…`) are always OK.
