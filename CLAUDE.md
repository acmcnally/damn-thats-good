...
- **No Linear issue IDs in code or UI copy.** Comments must be self-contained — git blame → PR → design doc is the trail back to the *why*. Never self-reference the issue being implemented; never cite a closed issue to explain existing behaviour; keep issue numbers out of rendered text (release names like "V2" are fine). In-repo pointers (`docs/features/<slug>/`, `docs/adr/…`) are always OK. **The one exception: a genuine future-work stub/seam** (an unwired prop, an inert control, a placeholder page, a documented constraint another issue will resolve) *should* cite it as **`TODO: DAMN-N — <what's missing>`** — greppable, and **deleted as part of that issue's work** when it ships.
...
- **Recipe Book** — owned by exactly one User (`DAMN-4`). Multi-owner "household" books (`DAMN-19`) were considered but are currently parked, unscheduled, out of every planned release. Contains Recipes.
...
- **Concurrent-edit handling is DECIDED (ADR-0007):** optimistic concurrency — a content save carries the base `current_version_id`, server rejects (412) if stale; non-versioned `Recipe` fields carry a separate row-version guard; on rejection the client preserves the draft and prompts to reconcile. This no-data-loss floor is permanent, regardless of release — the same recipe open in two tabs or two devices can race even with a single owner. An assisted-merge UI was planned as a V3 extension for multi-owner books; parked along with `DAMN-19`. Real-time/CRDT co-editing is rejected.
